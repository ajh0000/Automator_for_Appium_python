...
class Server:
	...
	def create_command_list(self,i,start_port,start_bootstrap_port):
		...
		if self.device_list != None:
			...
			#拼接Appium启动命令
			command = "appium -p {} -bp {} -U {} --no-reset --session-override --log E:/Appium/FXJC_Appium_Python/log/{}_log_{}".format(port_list[i], bootstrap_port_list[i], self.device_list[i], self.device_list[i], t)
